...
from llm0.kernels.rms_norm import TritonRMSNormFn, rms_norm_backward_triton, rms_norm_reference, rms_norm_triton
...
def compare_backward(dtype: torch.dtype, shape: tuple[int, ...]) -> None:
    ...
    y_tri.backward(grad)
    torch.cuda.synchronize()
    dx_kernel, dw_kernel = rms_norm_backward_triton(grad, x_ref.detach(), w_ref.detach(), 1e-5)
    torch.cuda.synchronize()

    x_err = float((x_ref.grad - x_tri.grad).abs().max())
    w_err = float((w_ref.grad - w_tri.grad).abs().max())
    kernel_x_err = float((x_ref.grad - dx_kernel).abs().max())
    kernel_w_err = float((w_ref.grad - dw_kernel).abs().max())
    print(f"backward dtype={dtype} shape={shape} x_err={x_err:.6e} w_err={w_err:.6e}")
    print(f"kernel_backward dtype={dtype} shape={shape} x_err={kernel_x_err:.6e} w_err={kernel_w_err:.6e}")
    ...
    if kernel_x_err > tolerance:
        raise AssertionError(f"kernel backward x_err {kernel_x_err} > {tolerance}")
    if kernel_w_err > tolerance * max(1, shape[0] if len(shape) == 2 else shape[0] * shape[1]):
        raise AssertionError(f"kernel backward w_err {kernel_w_err} is too large")
